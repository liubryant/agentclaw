...
class VipUpgradeBottomSheet : BottomSheetDialogFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.findViewById<TextView>(R.id.upgrade_title).text = when (quotaType) {
            QuotaType.VIDEO -> "今日视频次数已用完"
            QuotaType.IMAGE -> "今日图片次数已用完"
        }

        // Quota bar & text for video
        val videoBarView = view.findViewById<View>(R.id.quota_video_bar)
        val videoText = view.findViewById<TextView>(R.id.quota_video_text)
        ...
        view.findViewById<TextView>(R.id.quota_video_free_limit).text = "${QuotaManager.freeVideoLimit(ctx)}次/天"
        view.findViewById<TextView>(R.id.quota_video_vip_limit).text = "${QuotaManager.vipVideoLimit(ctx)}次/天"

        // Quota bar & text for image
        val imageBarView = view.findViewById<View>(R.id.quota_image_bar)
        val imageText = view.findViewById<TextView>(R.id.quota_image_text)
        ...
        view.findViewById<TextView>(R.id.quota_image_free_limit).text = "${QuotaManager.freeImageLimit(ctx)}张/天"
        view.findViewById<TextView>(R.id.quota_image_vip_limit).text = "${QuotaManager.vipImageLimit(ctx)}张/天"

        view.findViewById<Button>(R.id.upgrade_button).setOnClickListener {
            dismiss()
            startActivity(VipActivity.createIntent(requireContext()))
        }
        view.findViewById<TextView>(R.id.upgrade_cancel).setOnClickListener { dismiss() }
    }
